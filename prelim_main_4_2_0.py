...
from nodes_main_3_2_0 import new
#from itertools import combinations
from import_category6 import get_type_category_combinations_as_list, get_categorized_data, get_categorized_lists
...
def resolve4(df, control_number):
        ...
        if match.empty:
            ...
            """
            new_row = ['session_local_path_hash', 'run_recursion_counter', 'prelim_recursion_counter', 'prelim session not invoked', 'local_path_hash', 'response', 'api_keys[api_key_select]', str(time.time()), 'prelim session not invoked',
                       'prelim session not invoked', 'answer_counter2', 'prompt_counter', 'prelim session not invoked', 'prelim session not invoked', 'why_max', 'hierarchy', str(log_path), 'DebugG2', 'token_count', 'current_prompt', 'questions', 'prompt_counter', 'past_answer_load',
                       str(control_number), 'placeholder2', 'placeholder3']

            record_log(log_path, new_row)
            """
            return None
        ...
        if not answer_row.empty:
            ...
            if pd.isna(raw_answer) or raw_answer in [[], '', None]:
                """
                new_row = ['session_local_path_hash', 'run_recursion_counter', 'prelim_recursion_counter', 'prelim session not invoked', 'local_path_hash', 'response', 'api_keys[api_key_select]', str(time.time()), 'prelim session not invoked',
                           'prelim session not invoked', 'answer_counter2', 'prompt_counter', 'prelim session not invoked', 'prelim session not invoked', 'why_max', 'hierarchy', str(log_path), 'DebugG3', 'token_count', 'current_prompt', 'questions', 'prompt_counter', 'past_answer_load',
                           str(control_number), 'placeholder2', 'placeholder3']
            ...
            """
            new_row = ['session_local_path_hash', 'run_recursion_counter', 'prelim_recursion_counter', 'prelim session not invoked', 'local_path_hash', 'response', 'api_keys[api_key_select]', str(time.time()), 'prelim session not invoked',
                       'prelim session not invoked', 'answer_counter2', 'prompt_counter', 'prelim session not invoked', 'prelim session not invoked', 'why_max', 'hierarchy', str(log_path), 'DebugG4', 'token_count', 'current_prompt', 'questions', 'prompt_counter', 'past_answer_load',
                       str(control_number), 'placeholder2', 'placeholder3']
...
observation_data_file_name = "C:\\Users\\awanj\\GitHub\\disruptive_innovation\\observations_starter\\1 7.14.2025 observation_starter.csv"
...
combinations = all_combinations_with_retry(observation_keys_list[:21], 3, 1, initial_call=False)
"""
...
for observation in combinations:
    ...
    for question in questions:
        ...
        api_key1 = "AIzaSyD_osZZaEAfZnGs9ZvufvRv9XNohZM3AMY"  # squadronrenegade
        api_key2 = "AIzaSyCcg3FD-GuZqobt6fIq1iQGqI8NF9PKRTs"  # autonomousquality
        api_key3 = "AIzaSyCPMMmlzKZG2I8qT38t9roxXUTR3TjeX-w"  # autonomousquality
        api_key4 = "AIzaSyCN6gluCxVARUCaf2AyYYEfgn5283dMqOM"  # codingprofileforblogger@gmail.com
        ...
        response = ["prelim_generate_dummy_" + str(question_answer_counter_string)]
